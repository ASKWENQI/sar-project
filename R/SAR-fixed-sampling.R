...
neon_dob <- subset_samples(neon_dob,horizon == "M")
...
filtered_sites <- names(site_counts)[site_counts > 20]
...
for (i in 1:length(filtered_sites)){
  ...
  species <- vector(length = 20) # create a vector to save diversity
  for (j in 1:20){ 
  ...
  ex <- as.data.frame(cbind(species, "A"=c(1:20)))
  ex.list[[i]] <- ex
