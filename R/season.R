...
neon_season <- subset_samples(neon, sampleTiming == "peakGreenness")
# fallWinterTransition: mat_celsius, map_mm, temp_seasonality
# winterSpringTransition: variation among randomization
# peakGreenness: 1. perc_sansonality
d <- sample_data(neon_season) # sample data data frame
...
for (i in 1:length(a)){
  ...
  neon_sub <- subset_samples(neon_season, Site==a[i])
...
grid.arrange(p6, ncol = 3)

ggtrendline(x = species_sum, y = power.z[,1])+
  geom_point(aes(species_sum, power.z[,1]),color="black",pch=21,fill='white')+
  ylab("z value")
